...
chmod -R 0777 $SONARQUBE_HOME/conf
...
chmod -R 0555 $SONARQUBE_HOME/conf
...
# sonar app opt
if [ -z "${APP_OPT}" ]; then
  APP_OPT=" -Dsonar.log.console=true -Djava.security.egd=file:/dev/./urandom "
fi

DEFAULT_CMD=($JAVA_CMD $JAVA_OPT '-jar' ${SONARQUBE_HOME}/lib/sonar-application-${SONARQUBE_VERSION}.jar ${APP_OPT})
